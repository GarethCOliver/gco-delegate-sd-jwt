...
3. For each KB-SD-JWT except the final one:   
   a. Validate and process it according to [@!RFC9901] Section 7.1  
   ...
   b. Verify that there is exactly one disclosed element in the `delegate_payload` array.  
   c. If the `sd_hash` claim is present, calculate the digest over the proceeding SD-JWT or KB-SD-JWT and it's disclosures, as described in [@!RFC9901] Section 9.10.
...
4. For the final KB-SD-JWT:  
   a. Validate and process it according to 3a \- 3c  
   b. If the credential is a dSD-JWT then the type MUST be “kb-sd-jwt” otherwise it MUST be “kb-sd-jwt+kb”  
5. If the credential is a dSD-JWT+KB and Key Binding is required  
   a. Follow section 7.3 step 5 to to verify the KB-JWT, using the final KB-SD-JWT to retrieve the Delegate Holder public key.   
   b. If the `sd_hash` claim is present, calculate the digest over the proceeding SD-JWT or KB-SD-JWT+KB and it's disclosures as described in [@!RFC9901] Section 9.10.
